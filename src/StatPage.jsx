...
const StatPage = () => {
  ...
  const numberOfRegistrationFeedback = registrationsForExport.filter(
    (registration) => registration.registrationFeedback
  ).length;
  const numberOfTranslation = registrationsForExport.filter(
    (registration) => registration.translation
  ).length;
  const numberOfCancellation = registrationsForExport.filter(
    (registration) => registration.registrationFeedback && !registration.onsite
  ).length;
  ...
  const onsiteBreakoutSessions = breakoutSessions.filter(
    (bs) => bs.numberOfRegistration > 0
  );

  const csvReport = {
    data: registrationsForExport,
    headers: headers,
    filename: "iok2023_jelentkezesek.csv",
  };
  ...
  return (
    <>
      ...
      <div>&nbsp;</div>
      {onsiteBreakoutSessions?.map((breakoutSession) => (
        <div>
          {breakoutSession.name}: {breakoutSession.numberOfRegistration}{" "}
        </div>
      ))}
      <div>&nbsp;</div>
      <div>
        Helyszíni résztvevő visszajelzés: {numberOfRegistrationFeedback}
      </div>
      <div>Ebből ennyi a lemondás: {numberOfCancellation}</div>
      <div>Tolmácsolást kér: {numberOfTranslation}</div>
      <CSVLink {...csvReport} separator=";" style={{ textDecoration: "none" }}>
        <Button variant="outlined">Exportálás CSV fájlba</Button>
      </CSVLink>
    </>
  );
};
